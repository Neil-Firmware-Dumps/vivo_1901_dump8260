...
function zram_writeback_config(){
	...
	# just >rom_12.0 or special project, support zram writeback
	rom_version=`getprop ro.vivo.os.version`
	if [ `expr $rom_version \< 12.0` -eq 1 ] && [ "$special_project" == "0" ]; then
		return
	fi
	...
	if [ -d /sys/ufs ]; then
		...
		life_time_a=`cat /sys/devices/platform/bootdevice/mmc_host/mmc0/mmc0:0001/life_time | awk '{print $1}'`
		life_time_b=`cat /sys/devices/platform/bootdevice/mmc_host/mmc0/mmc0:0001/life_time | awk '{print $2}'`
	fi
